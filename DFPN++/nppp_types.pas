...
{$ELSE}
  ...
  nppPChar = PAnsiChar;
{$ENDIF}
  TSciCursorPointer = (scpNormal = SC_CURSORNORMAL, scpWait = SC_CURSORWAIT);
  TNppView = (npvMain = MAIN_VIEW, npvSub = SUB_VIEW);
  TSciAutoListSortOrder = (
    asoPresorted = SC_ORDER_PRESORTED,
    asoPerformSort = SC_ORDER_PERFORMSORT,
    asoCustom = SC_ORDER_CUSTOM
  );
  TSciUpdateUIFlag = (
    uuiContent = SC_UPDATE_CONTENT,
    uuiSelection = SC_UPDATE_SELECTION,
    uuiVScroll = SC_UPDATE_V_SCROLL,
    uuiHScroll = SC_UPDATE_H_SCROLL
  );
  // list extracted from Notepad_plus_msgs.h (source code of NPP 5.2)
  ...
  _TFuncItem = record
    ItemName: Array[0..MAXFUNCNAMELENGTH-1] of nppChar;
    Func: PFUNCPLUGINCMD;
    CmdID: Integer;
    Checked: Boolean;
...
constructor TShortcutKey.Create(IsCtrl, IsAlt, IsShift: Boolean; Key: Byte);
begin
  Self.IsCtrl := IsCtrl;
  Self.IsAlt := IsAlt;
